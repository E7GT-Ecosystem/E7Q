...
The executable runner first checks the E7C checkout SHA and supplies its actual `admit` function to the mapping, so E7C source admission validates the document *first*. `map_joint` itself accepts a caller-provided admission callback: outside this pinned runner, trust in that callback is an explicit precondition, not an authentication guarantee supplied by the generic mapping API. The narrower pilot mapping additionally admits one named intent, binary graph pairs with exact known `pilot.layout` and `pilot.target` tags, layout edges `[]`/`[AB]`, target edges `[]`/`[BC]`, at most four unique canonically ordered support rows, and reduced nonzero signed fractions. `L1` means coordinate 0 contains `AB`; `T1` means coordinate 1 contains `BC`. The two predicates remain E7C `FG3-JOINT-COORD0-ABSENT-AB/0.1-provisional` and `FG3-JOINT-COORD1-ABSENT-BC/0.1-provisional`. Every mapping row binds its original source index, source row digest, Q-A1 member ID and rational numerator/positive denominator. Revalidation rebuilds all IDs and rejects stale family, altered row order/coefficient, missing row, unknown member or ambiguous mapping. Member sorting in Q-A1 is kept distinct from canonical source Joint order.
